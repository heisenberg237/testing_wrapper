...
class AGBParser(BaseParser):
    """Parser for CZT supplier data."""
    mledata_db = create_mongo_client(config.mongo.uri)[config.mongo.database_mledata]
    mletracking_collection = mledata_db[config.mongo.collection_mletracking]
    ...
    def parse(self, file_path: Path) -> InputData:
        ...
        df = self._apply_CZT_transformations(df)

        metadata = self._extract_metadata(file_path)
        metadata["supplier"] = "CZT"
    ...
    def _extract_piece_metadata(self, piece_folder: Path) -> PieceMetadata:
        ...
        self.serial = info.get("serial_number") or self._guess_serial_number(name, tokens)
    ...
    def _validate_file_format(self, df: pd.DataFrame) -> None:
        ...
        if missing:
            raise ValueError(f"CZT format missing required columns: {missing}")

    def _apply_CZT_transformations(self, df: pd.DataFrame) -> pd.DataFrame:
        # --- ensure numeric ---
        for col in ["x", "y", "z"]:
